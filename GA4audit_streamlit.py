...
def audit_video_interaction_url(
    ...
    if matched_video_event:
        ...
        result["pageview_event_json"] = "{}"
        result["capture_failed"] = True
        result["capture_failure_reason"] = "video_interaction was not observed during the bounded video capture pass."
        result["status"] = "FAIL"
        result["issues"] = result["capture_failure_reason"]
...
def build_default_article_detail_video_rules(template: Optional[dict]) -> List[dict]:
    template_id = str((template or {}).get("template_id") or "").strip()
    base_rule_id = f"{template_id or 'article_detail'}__video_default"
    return [
        {
            "rule_id": f"{base_rule_id}_event",
            "template_id": template_id,
            "rule_scope": "event",
            "field_name": "video_interaction",
            "rule_type": "exact",
            "expected_values": "video_interaction",
        },
        {
            "rule_id": f"{base_rule_id}_dynamic_video_embed_type",
            "template_id": template_id,
        ...
        },
        {
            ...
            "rule_type": "one_of",
            "expected_values": "automatic|manual",
        },
        {
            "rule_id": f"{base_rule_id}_position_fold",
            "template_id": template_id,
            "rule_scope": "execution",
        ...
        {
            "rule_id": f"{base_rule_id}_video_orientation",
            "template_id": template_id,
            "rule_scope": "execution",
            "field_name": "video_orientation",
            "rule_type": "one_of",
            "expected_values": "horizontal|vertical",
        },
        {
            "rule_id": f"{base_rule_id}_video_percent",
            "template_id": template_id,
            "rule_scope": "execution",
...
def should_use_ga_related_video_capture(normalized_url: str) -> bool:
...
def capture_related_video_event_for_ga(url: str, headless: bool = True) -> Dict[str, Any]:
    from video_event_mvp import (
        ...
        normalized_has_field,
        reset_visible_videos,
        safe_quit,
        scroll_to_related_video_embed,
    )
    ...
    try:
        ...
        debug_steps.append(
            {
                ...
                "primary_target_found": False,
                "target": "related_embed",
                "source": "ga_streamlit_bottom_video",
            }
        )
        ...
        while time.time() - start < 8:
            ...
            if (
                normalized["gtag_video_events"]
                or normalized["data_layer_video_events"]
                or normalized["transport_video_events"]
            ):
                matched = normalized
                if first_match_at is None:
                    first_match_at = time.time()
                if normalized_has_field(normalized, "dynamic_video_embed_type") or time.time() - first_match_at >= 2:
                    break
            time.sleep(1.0)
    ...
    if capture_video_clicked:
        ...
        if selected_template is None:
            ...
            try:
                ...
                if should_use_ga_related_video_capture(normalized_url):
                    mvp_video_result = capture_related_video_event_for_ga(normalized_url, headless=True)
                else:
